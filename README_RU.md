...
Наще хранилище это объект с состоянием программы, который мы импортируем в каждый модуль и меняем по ссылке(изменения применяются везде).
...
```
### Маршрутизация
У нас маршрутизация на стороне клиента, которую контроллирует `./client/spa/router.js` и `./client/spa/spaControl.js`.
...
массив функций-обработчиков этого события. Все обработчики событий выпполняются асинхронно и последовательно.
В данном случае, наша страничка обрабатывает всего две кнопки, соответствующие обработчики объявлены как методы класса. Если у контроллера есть обработчики — онв возвращает массив функций в `main.js` где они вызываются.

**!Примечание:** В контроллерах может бысть любой тип конфигурации, в зависимости от обрабатываемых событий: `changeConfig`, `inputConfig`, `clickConfig` и др.
...
В общем, взаимодействие и синхронизация пользователей основаны на рассылке событий всем пользователям. Сервер ничего не знает о состоянии игры, поэтому игру контроллирует ведущий. Большинство событий генерирует ведущий и отправляет всем игрокам в комнате.
...
```
В начале файла стоят константы, что контроллируют минимальное количество игроков, время на ход, время на аппеляцию и время на игру. 
``` js
// game_class.js
...
Каждое другое действие над игровым интерфейсом контроллирует `gameField.js`:
- изменение разметки
...
### Сервер
Сервер написан на простом js, без использования сторонних фреймворков(не считая node js).  
Чтобы запуситть сервер создайте объект класса [Server](./server/server.js) и передайте номер нужного вам порта ([пример](index.js)). Класс Server — синглтон.  
...
Пользователи онлайн хранятся в поле `_users` класса `Server`. Для каждого пользователя мы генерируему уникальный id, и все сохраняются в объекте с такой структурой:  
``` js
